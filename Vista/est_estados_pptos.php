<?php
	...
	if(!empty($_POST["user"])){
		...
		if($turno == 1){
			...
			$sql_ejecutivos = mysql_query("select distinct pr.id_procliente,pr.nombre_producto
			...
			and ot.producto_clientes_codigo_PRC = pr.id_procliente order by pr.nombre_producto asc");
			while($row = mysql_fetch_array($sql_ejecutivos)){
				$list_ejecutivos.= "<option value=".$row['id_procliente'].">".utf8_decode($row['nombre_producto'])."</option>";
			}
			$estructura = $list_ejecutivos;
		}
	}	
