<?php
	...
	switch ($turno){
		...
		case 2:
			...
			}else if($_POST['por_donde'] == 2){
				if($_POST['num_asi'] > 0){
					for($i = 0; $i < $_POST['num_asi']; $i++){
						$t->insert_personas_asires($_POST["asi$i"],"ASI",$_POST['idot'],$num);
					}
				}
				
				if($_POST['num_res'] > 0){
					for($i = 0; $i < $_POST['num_res']; $i++){
						$t->insert_personas_asires($_POST["resp$i"],"RES",$_POST['idot'],$num);
					}
				}
			}			
			
			if($_POST['tipo'] == 33){
				mysql_query("update cabpresup set estado_presup = '5' where ot = '$nombre_ot' and estado_presup = '4'");
			}
			mkdir("../Process/OT/$nombre_ot/TAREAS/$num");
			if($_POST['tipo_envio'] == 2){
				for($tx = 0; $tx < $_POST['num_arc']; $tx++){
					$file = $_FILES['arc'.$tx]['name'];
					$file2 = $_POST['name_arc_real'.$tx];
					move_uploaded_file($_FILES['arc'.$tx]['tmp_name'],"../Process/OT/$nombre_ot/TAREAS/$num/".$file2);
				}
			}			
			echo $num;
			break;
		...
		case 3:
			...
			if($_POST['test'] == 1){
				...
				$t->actualizar_respuesta($_POST['id'],$fecha2,date("Y-m-d h:i:s"),$_POST['usu']);
				$t->set_codigo_ot_tarea($nombre_ot);
				$num = $t->codigo_automatico_tarea();
				$t->set_codigo_area_tarea($_POST['depto']);
				...
				if($_POST['num_ppto'] == 0){
					...
					$t->insert_tarea2($_POST['usu'], date("Y-m-d h:i:s"),$num,"R",$_POST['id'],date("h:i:s"),$pptox,$hm_militar);
				}else{
					$t->insert_tarea2($_POST['usu'], date("Y-m-d h:i:s"),$num,"R",$_POST['id'],date("h:i:s"),$_POST['num_ppto'],$hm_militar);
				}
		...
		case 5:
			...
			$clie = $_POST['clie'];
			$usu = $_POST['usu'];
			$consulta_clie = "select  distinct e.id_procliente, e.nombre_producto from producto_clientes e, pcliepro p where e.estado = 1 and 
