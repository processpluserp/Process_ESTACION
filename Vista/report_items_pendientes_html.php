<?php
	...
	<div style='background-color:#E5E5E5; border-radius: 15px; width: 1800px;'>
	<table width = '100%' class = 'tabla_reporte_estados_pptos tabla_reportes4 tabla_reportes'>";
	$estructura_tabla .= "<thead>";
	$estructura_tabla .= "<tr>";
	for($i = 0; $i < count($items_pendiente_ordenacion); $i++){
		if ($i<1) {
			$estructura_tabla.="<th style='background-color:#DAEFB0; color: rgb(83, 86, 78); border-color: #FFFFFF; height: 35px;' class = 'th_report' nowrap>".$items_pendiente_ordenacion[$i]."</th>";		
		}else
			$estructura_tabla.="<th style='background-color:#F7EFB0; color: rgb(83, 86, 78); border-color: #FFFFFF; height: 35px;' class = 'th_report' nowrap>".$items_pendiente_ordenacion[$i]."</th>";
	...
	if(!empty($_POST["user"]) && !empty($_POST["emp"]) && !empty($_POST["director"]) && !empty($_POST["ejecutivo"]) && !empty($_POST["turno"])){
		...
		ip.dias,ip.q,ip.val_item,ip.por_prov,ip.iva_item,ip.name_item
		from cabot ot, clientes c, producto_clientes pr,cabpresup p,itempresup ip,proveedores pp
		...
		and p.codigo_presup = ip.ppto and ip.pk_orden = '0' and ip.proveedor = pp.codigo_interno_proveedor $sql_cliente $sql_ppto
		order BY c.nombre_comercial_cliente,pr.nombre_producto,ot.codigo_ot,ip.pk_orden");
		...
		while($row = mysql_fetch_array($sql_nivel_cliente)){
			...
			$val_inicial = $row['dias']*$row['q']*$row['val_item'];
			$iva = $val_inicial * ($row['iva_item']/100);
			$vol = $val_inicial * ($row['por_prov']/100);
			...
			$array_cliente[$i][14] = $row['name_item']; //ITEM
			$array_cliente[$i][15] = $row['dias'];
		...
		for($n = 0; $n < count($array_f_clientesx); $n++){
			for($c = 0; $c < count($array_cliente); $c++){
				if( $array_f_clientesx[$llaves_clientes[$n]] == $array_cliente[$c][10]){
					...
					if ($proveedor!="") {
						$estructura_tabla .= "<td ROWSPAN=".count($array_cliente[$c][14])." style = 'vertical-align:top;' nowrap>".$proveedor."</td>";
					}							
						...
						$estructura_tabla .= "<td style = 'vertical-align:top;text-align:center;' nowrap>".$array_cliente[$c][16]."</td>";
						$estructura_tabla .= "<td style = 'vertical-align:top;text-align:center;' nowrap>$ ".$array_cliente[$c][17]."</td>";
			...
			/*$estructura_tabla.="<tr><td style = 'border-bottom:1px solid black;' colspan = '18'><br></br></td></tr>
			</tbody></table>
					</div>";
			*/
