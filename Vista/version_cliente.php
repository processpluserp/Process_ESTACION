<?php
	...
	if($_POST['ppto'] == 0){
		...
		mysql_query("START TRANSACTION");
			...
			$slq_items = mysql_query("select name_grupo,name_item,descripcion2,pk_item,proveedor,celula,dias,q,descripcion,val_item,iva_item,fecha_ant,por_ant,cliente,val_desde_item,por_prov,usuario,fecha_registro,ppto,asoc,num_interno,vc
			from itempresup where pk_orden = '0' and ppto = '$ppto' and vi = '$version' and vc = '$versionc'");
			while($row = mysql_fetch_array($slq_items)){
				mysql_query("insert into itempresup(name_grupo,name_item,descripcion2,pk_item,proveedor,celula,dias,q,descripcion,val_item,iva_item,fecha_ant,por_ant,cliente,val_desde_item,por_prov,usuario,fecha_registro,ppto,asoc,num_interno,vi,vc) values
				('".$row['name_grupo']."','".$row['name_item']."','".$row['descripcion2']."','".$row['pk_item']."','".$row['proveedor']."','".$row['celula']."','".$row['dias']."','".$row['q']."','".$row['descripcion']."','".$row['val_item']."','".$row['iva_item']."','".$row['fecha_ant']."','".$row['por_ant'].
