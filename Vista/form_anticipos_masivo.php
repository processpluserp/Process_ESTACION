<?php
	...
	if($tipo == 1){
		...
		$sql_item = mysql_query("select id,dias,q,name_item,name_grupo,asoc,val_item,iva_item
		from itempresup where ppto = '$ppto' and vi = '$vi' and vc = '$vc' and pk_op = 0 and pk_orden = 0");
		while($row = mysql_fetch_array($sql_item)){
			...
			$est_anticipo.="<tr>
				<td >
					<div>
						<input type = 'checkbox' checked id = 'item_anticipo".$row['id']."' name = 'item_anticipo[]' value = '".$row['id']."' class = 'radio'/>
						<label for='item_anticipo".$row['id']."'><span><span></span></span></label>
					</div>
				</td>
				<td align = 'left'>".utf8_decode($row['name_grupo'])."</td>
				<td align = 'left'>".utf8_decode($row['name_item'])."</td>
				<td >".$row['dias']."<span class = 'hidde' id = 'ant_dias".$row['id']."'>".$row['dias']."</span></td>
				<td >".$row['q']."<span class = 'hidde' id = 'ant_q".$row['id']."'>".$row['q']."</span></td>
				...
				<td >".number_format($libre)."<span id = 'ant_val_unitario".$row['id']."' class = 'hidde'>".$libre."</span></td>
				<td >
					<input type = 'text' min = '1' value = '0' id = 'por_ant".$row['id']."'   onkeyup = 'validar_porcentaje_libre_item_anticipo(".$row['id'].",$libre)' onchange = 'validar_porcentaje_libre_item_anticipo(".$row['id'].",$libre)' />
					<span id = 'valor_real_solicitado".$row['id']."' class = 'hidde'>0</span>
				</td>
				<td >
					<span id = 'total_solicitado".$row['id']."'></span>
				</td>
			</tr>";
		}
		$est_anticipo."</table>";
